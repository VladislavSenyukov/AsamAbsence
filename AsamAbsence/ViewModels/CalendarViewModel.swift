...
class CalendarViewModel {
    ...
    private var absenceMap: [CalendarDate: Absence] = [:]
    ...
    func fetchAbsenceData() {
        delegate?.showLoading(true)
        absenceManager.fetchAbsenceData { [weak self] absenceData in
            self?.absenceMap = absenceData.reduce([:]) { (result, absence) in
                let next = absence.dates
                    .reduce(into: [CalendarDate: Absence]()) { $0[$1] = absence }
    ...
    func cellItemDataForIndexPath(_ indexPath: IndexPath) -> CalendarCellItemData {
        ...
        if dayIndex > additionalDays, let monthIndex = daysLayoutPrefixSum.firstIndex(where: { $0 >= dayIndex }) {
            ...
            dataItem = CalendarCellItemData(type: .date(cellItemDate),
                                            isSelected: false,
                                            absenceType: absenceMap[cellItemDate]?.type)
        } else {
    ...
    func absenceDataForIndexPath(_ indexPath: IndexPath) -> Absence? {
        let cellData = cellItemDataForIndexPath(indexPath)
        switch cellData.type {
        case .empty:
            return nil
        case .date(let caledarDate):
            return absenceMap[caledarDate]
        }
    }
    ...
    func removeDayWithIndexPath(_ indexPath: IndexPath, from absence: Absence) {
        let cellData = cellItemDataForIndexPath(indexPath)
        guard
            case .date(let date) = cellData.type,
            let dateIndex = absence.dates.firstIndex(of: date) else {
            return
        }
        var absence = absence
        absence.dates.remove(at: dateIndex)
        delegate?.showLoading(true)
