...
extension MenuViewController: UITableViewDelegate {
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        switch menuOption {
        case .calendar:
            guard let calendarVC = AsamAbsenceApp.shared.makeCalendarVC() else {
                return
            }
            navigationController?.pushViewController(calendarVC, animated: true)
        case .profile:
            guard let profileVC = AsamAbsenceApp.shared.makeProfileVC() else {
                return
            }
            navigationController?.pushViewController(profileVC, animated: true)
        case.logout:
            showLoading(true)
            AsamAbsenceApp.shared.userManager.logout { [weak self] in
